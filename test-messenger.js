...
const govDecrypt = async function (secret, [header, ct]) {
  ...
  /* 
  vGov: The public key of the government entity.
  cGov: The encrypted message key (cipher key) encrypted using the derived govKey.
  ivGov: The initialization vector used for encryption.
  receiverIV: The initialization vector used for encryption of the actual message.
  */
  let govKey = await computeDH(secret, header.vGov)
  //decryption key generation for Gov side
  ...
  /* 
  In the second step, the decrypted message key (mk) is imported as a SubtleCrypto key using the importKey method.
The importKey method takes parameters such as the key format (raw in this case), the key material (the decrypted message key mk), the algorithm (AES-GCM), whether the key should be extractable (true in this case), and the key usages (both 'encrypt' and 'decrypt' in this case, indicating that the key will be used for both encryption and decryption operations).
This step prepares the decrypted message key (subtleMK) for use in subsequent cryptographic operations, allowing it to be used directly with the Web Cryptography API's SubtleCrypto methods for encryption and decryption.
  */
...
describe('Messenger', function () {
  ...
  describe('functionality', function () {
    ...
    it('imports a certificate without an error', async function () {
      ...
      await alice.generateCertificate('alice') // alice generating her certificate
      const bobCertificate = await bob.generateCertificate('bob') //bob generating his certificate
      const bobCertSignature = await signWithECDSA(caKeyPair.sec, stringifyCert(bobCertificate)) //bob signing -> digital sign 
      await alice.receiveCertificate(bobCertificate, bobCertSignature) // sending the certificate to verify
    })
    ...
    it('generates an encrypted message without error', async function () {
      ...
      caKeyPair.pub refers to the public key of the certificate authority, and govKeyPair.pub refers to the public key of the government entity. By passing these keys during the instantiation of MessengerClient, the client is initialized with the necessary keys to verify certificates and potentially interact with the government entity.
      */
      const bob = new MessengerClient(caKeyPair.pub, govKeyPair.pub)
